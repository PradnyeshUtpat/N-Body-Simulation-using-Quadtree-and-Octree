//Barnes Hut for 2D system
//4 Quadrants
//Implementation using Quad trees
...
//Initiate the new Barnes Hut 2D node
//Takes lower and higher bounds of the system as parameters
//Initialize centre of mass , leaves and charge of the system to 0
quadtree* BarnesHut2D_init(float x1, float y1, float x2, float y2){
    ...
    //bounds decide the octants to which the node belongs
    ...
    //Initialize the centre of mass and the total charge
    root->com_x = 0;
    root->com_y = 0;
    root->charge = 0;
    ...
    for (int i = 0; i < 4; i++)
        root->child[i] = NULL;
    //Initialize the leaves to 0 as no nodes have been added    
    root->leaves = 0;
    return root;
}

//Delete the entire tree after the simulattion has been performed
void BarnesHut2D_destroy_tree(quadtree *root){
    if (!root) 
        return;//No need to delete the tree if already it doesn't exist
    //Recursively deletion of the tree
    for (int i = 0; i < 4; i++)
        BarnesHut2D_destroy_tree(root->child[i]);
    free(root);
}

//Insert a node in the quad tree
int BarnesHut2D_insert(quadtree *root, float x, float y, float c){
    if (!root) 
        return 0;
    //Insertion when the tree is empty i.e. thenumber of leaves is zero
    if (root->leaves == 0) {
        root->com_x = x;
        root->com_y = y;
        root->charge = c;
    }
    //If the tree has atleast one node i.e. the tree is not empty
    else {
        //for a single node
        if (root->leaves == 1) {
